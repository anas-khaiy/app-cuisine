...
export class NavbarComponent {
  ...
  constructor()
  {}
   searchFunction(e:any,searchBar:string) {

    let listeFoods:Food[]=[]
    e.preventDefault()
    const htmlDiv=document.getElementById("mainContainer") as HTMLElement
    if(searchBar=="" || searchBar=="." || searchBar=="*")
    {
      htmlDiv.innerHTML="<div class='alert alert-danger mt-3'  role='alert'>Please Insert Your Search!</div>"
    }
    else
    {
      var textFinal="<div class='row'>"

      this.listFoods.filter(e=> new RegExp("\\b"+searchBar.toLowerCase().trim()+"\\b").test(e.titleFood.toLowerCase())).forEach(e=>{
        listeFoods.push(e)
        textFinal+="<div class='col-xs-12 col-lg-4 col-md-4 col-sm-12 mt-3'> <div class='card' style='height: 350px;'><img src="+e.imgFood+" class='card-img-top' style='height: 200px; object-fit: cover; object-position: center;' alt=''><h5 class='card-title m-auto mt-2'>"+e.titleFood+"</h5><div class='card-body'><p class='card-text'>"+e.descFood+"</p></div></div></div>"
      })
  
      textFinal+="</div> </div>"
  
      if(listeFoods.length>0)
      {
        htmlDiv.innerHTML=textFinal
      }
      else
      {
        htmlDiv.innerHTML="<div class='alert alert-danger mt-3'  role='alert'>Not Found!</div>"
      }
    }
